...
#define BUZZER_PIN 26 // BCM 핀 번호
#define CONSUMER "buzzer_app"

int doReMi[8] = {523, 587, 659, 698, 783, 880, 987, 1046};
void softwarePWM(struct gpiod_line *line, int freq, int duration_ms);
...
int main(void)
{
    ...
    while (1)
    {
        for (int i = 0; i < 8; ++i)
        {
            softwarePWM(buzzer_line, doReMi[i], 100);
        }
    }
...
void softwarePWM(struct gpiod_line *line, int freq, int duration_ms)
{
    ...
    int duty_us = period_us / 2; // 50% 듀티 사이클
